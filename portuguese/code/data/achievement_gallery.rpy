translate portuguese strings:

    # game/code/data/achievement_gallery.rpy:5
    old "01: Scratching the Surface"
    new "01: É apenas o começo"

    # game/code/data/achievement_gallery.rpy:5
    old "Finish the game."
    new "Termine o jogo."

    # game/code/data/achievement_gallery.rpy:5
    old "02: Dulled Euphoria"
    new "02: Euforia Reduzida"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Iona's path."
    new "Jogue o caminho da Iona."

    # game/code/data/achievement_gallery.rpy:5
    old "03: Monica's Melons"
    new "03: Melões da Monika"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Monica's path."
    new "Jogue o caminho da Monika.."

    # game/code/data/achievement_gallery.rpy:5
    old "04: Queen Bae"
    new "04: Rainha"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Isa's path."
    new "Jogue o caminho da Isa."

    # game/code/data/achievement_gallery.rpy:5
    old "05: Down With the Bourgeoisie"
    new "05: Chega da Burguesia"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Sam's path."
    new "Jogue o caminho da Sam."

    # game/code/data/achievement_gallery.rpy:5
    old "06: Blood and Boners"
    new "06: Sangue e Osso"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Delphia's path."
    new "Jogue o caminho da Delphia."

    # game/code/data/achievement_gallery.rpy:5
    old "07: Bagging Bookworms"
    new "07: Pegando Nerds"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Irene's path."
    new "Jogue o caminho da Irene."

    # game/code/data/achievement_gallery.rpy:5
    old "08: Innocence Irrelevant"
    new "08: Inocência Irrelevante"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Lumi's path."
    new "Jogue o caminho da Lumi."

    # game/code/data/achievement_gallery.rpy:5
    old "09: Strokes on Canvas"
    new "09: Manchando o Canvas"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Ezra's path."
    new "Jogue o caminho da Ezra."

    # game/code/data/achievement_gallery.rpy:5
    old "10: Wine Mom"
    new "10: Mamãe do Vinho"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Sally's path."
    new "Jogue o caminho da Sally."

    # game/code/data/achievement_gallery.rpy:5
    old "11: Fleur de Passion"
    new "11: Fleur de Passion"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Fleur's path."
    new "Jogue o caminho da Fleur."

    # game/code/data/achievement_gallery.rpy:5
    old "12: Prescription Desires"
    new "12: Desejos na Medida"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Asami's path."
    new "Play Asami's path."

    # game/code/data/achievement_gallery.rpy:5
    old "13: What Did the Fox Moan"
    new "13: Qual o gemido da Raposa"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Foxy's path."
    new "Jogue o caminho da Foxy."

    # game/code/data/achievement_gallery.rpy:5
    old "14: Mighty Morgan"
    new "14: Grande Morgan"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Morgan's path."
    new "Jogue o caminho da Morgan."

    # game/code/data/achievement_gallery.rpy:5
    old "15: Feared Straight"
    new "15: O Medo dá Jeito"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Corinne's path."
    new "Jogue o caminho da Corinne."

    # game/code/data/achievement_gallery.rpy:5
    old "16: Juggling Favors"
    new "16: Troca de Favores"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Nirmala's path."
    new "Jogue o caminho da Nirmala."

    # game/code/data/achievement_gallery.rpy:5
    old "17: Lifelong Longing"
    new "17: Amor Duradouro"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Josie's path."
    new "Jogue o caminho da Josie."

    # game/code/data/achievement_gallery.rpy:5
    old "18: And He Was Born"
    new "18: E Ele Nasceu"

    # game/code/data/achievement_gallery.rpy:5
    old "Select your name."
    new "Escolha seu nome."

    # game/code/data/achievement_gallery.rpy:5
    old "19: The True Beginning"
    new "19: O Verdadeiro Começo"

    # game/code/data/achievement_gallery.rpy:5
    old "Reach the end of the prologue."
    new "Chegue no final do prólogo."

    # game/code/data/achievement_gallery.rpy:5
    old "20: Saving Private Josie"
    new "20: O Resgate da Soldada Josie"

    # game/code/data/achievement_gallery.rpy:5
    old "Save Josie from drowning."
    new "Salve Josie de se afogar."

    # game/code/data/achievement_gallery.rpy:5
    old "21: On to the Tournament"
    new "21: No Torneio"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Winner's Part 2."
    new "Terminou a Parte 2 dos Vencedores."

    # game/code/data/achievement_gallery.rpy:5
    old "22: On to the Forest"
    new "22: Para a Floresta"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Cult's Part 2."
    new "Terminou a Parte 2 do Culto."

    # game/code/data/achievement_gallery.rpy:5
    old "23: On to the Compound"
    new "23: Para o Esconderijo"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Syndicate's Part 2."
    new "Terminou a Parte 2 do Sindicato."

    # game/code/data/achievement_gallery.rpy:5
    old "24: Injecting the L"
    new "24: Injetando Derrota"

    # game/code/data/achievement_gallery.rpy:5
    old "Lose the Winner's tournament."
    new "Perca o campeonato dos Vencedores."

    # game/code/data/achievement_gallery.rpy:5
    old "25: Heart Shaped Box"
    new "25: Caixa de Coraçãozinho"

    # game/code/data/achievement_gallery.rpy:5
    old "Experience spiritual awakening with Nia."
    new "Passe por um despertar espiritual com a Nia."

    # game/code/data/achievement_gallery.rpy:5
    old "26: Kablooey"
    new "26: Kabooom"

    # game/code/data/achievement_gallery.rpy:5
    old "See the Wasp's warehouse explosion."
    new "Veja a explosão do armazém das Vespas."

    # game/code/data/achievement_gallery.rpy:5
    old "27: Threesome Therapy"
    new "27: Terapia a Três"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Nia and Silvia."
    new "Faça sexo a três com Nia e Silvia."

    # game/code/data/achievement_gallery.rpy:5
    old "28: Temptations in Ink"
    new "28: Tentações de Tatuagens"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Nia and Zemfira."
    new "Faça sexo a três com Nia e Zemfira."

    # game/code/data/achievement_gallery.rpy:5
    old "29: Rebel Without a Ball"
    new "29: Rebelde sem Bola"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Zemfira and Silvia."
    new "Faça sexo a três com Zemfira e Silvia."

    # game/code/data/achievement_gallery.rpy:5
    old "30: Hacker's Delight"
    new "30: Olha o Hacker"

    # game/code/data/achievement_gallery.rpy:5
    old "Click the Butcher's street background character."
    new "Clique no personagem de fundo na rua do Açougue."

    # game/code/data/achievement_gallery.rpy:5
    old "31: Meat Lovers"
    new "31: Amante de Carne"

    # game/code/data/achievement_gallery.rpy:5
    old "Win Greg's minigame."
    new "Vença o minigame do Greg."

    # game/code/data/achievement_gallery.rpy:5
    old "32: Top of the Class"
    new "32: Melhor da Classe"

    # game/code/data/achievement_gallery.rpy:5
    old "Earn 18+ points with a group."
    new "Ganhe 18+ pontos com um grupo."

    # game/code/data/achievement_gallery.rpy:5
    old "33: Eskimo Sisters"
    new "33: Irmãs Esquimó"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with Monika and kiss Olesya."
    new "Faça sexo com Monika e beije Olesya."

    # game/code/data/achievement_gallery.rpy:5
    old "34: The Truth Will Free You"
    new "34: A Verdade te Libertará"

    # game/code/data/achievement_gallery.rpy:5
    old "Kiss Pheobe and tell Fleur and Asami the truth."
    new "Beije Phoebe e fale para a verdade para Fleur e Asami."

    # game/code/data/achievement_gallery.rpy:5
    old "35: Junior Detective"
    new "35: Detetive Júnior"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock a character on the investigation board."
    new "Libere um personagem no quadro de investigação."

    # game/code/data/achievement_gallery.rpy:5
    old "36: Privates Investigator"
    new "36: Investigador de Particulares"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock 20 characters on the investigation board."
    new "Libere 20 personagens no quadro de investigação."

    # game/code/data/achievement_gallery.rpy:5
    old "37: Master Sleuth"
    new "37: Detetive Mestre"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock 40 characters on the investigation board."
    new "Libere 40 personagens no quadro de investigação."

    # game/code/data/achievement_gallery.rpy:5
    old "38: Passion Detective"
    new "38: Detective do Amor"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with 10 girls."
    new "Faça sexo com 10 garotas."

    # game/code/data/achievement_gallery.rpy:5
    old "39: Campus Casanova"
    new "39: Casanova do Campus"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with all the girls."
    new "Faça sexo com todas as garotas."

    # game/code/data/achievement_gallery.rpy:5
    old "40: Professor's Pet"
    new "40: Bichinho da Professora"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Diana and Ronja's paths."
    new "Jogue os caminhos da Diana e Ronja."

    # game/code/data/achievement_gallery.rpy:5
    old "41: Fetish Locator"
    new "41: Fetish Locator"

    # game/code/data/achievement_gallery.rpy:5
    old "Experience all fetishes at the party."
    new "Veja todos os fetiches da festa."

    # game/code/data/achievement_gallery.rpy:5
    old "42: Calm and Collected"
    new "42: Calmo e Sério"

    # game/code/data/achievement_gallery.rpy:5
    old "Throw Brigitta into the pool."
    new "Jogue Brigitta na piscina."

    # game/code/data/achievement_gallery.rpy:5
    old "43: Yellow Balling"
    new "43: Na Vontade"

    # game/code/data/achievement_gallery.rpy:5
    old "Get blue balled by Nia and Zemfira at the party."
    new "Seja deixado na vontade pela Nai e Zemfira."

    # game/code/data/achievement_gallery.rpy:5
    old "44: Escaping the Matrix"
    new "44: Escapando the Matrix"

    # game/code/data/achievement_gallery.rpy:5
    old "Click the unclickable option in the nightmare."
    new "Clique na opção impossível de clicar no pesadelo.

    # game/code/data/achievement_gallery.rpy:5
    old "45: Behind the Scenes"
    new "45: Nos Bastidores"

    # game/code/data/achievement_gallery.rpy:5
    old "Play postcredit scene."
    new "Jogue a cena pós-crédito."

